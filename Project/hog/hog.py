...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    points = 3 * abs(player_score % 10 - opponent_score % 100 // 10)
    if points == 0:
        return 1
    else:
        return points 
...
def make_averaged(original_function, times_called=1000):
    ...
    def averaged(*args):
        ...
        while i <= times_called:
            cnt = cnt + original_function(*args)
            i = i + 1
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    while num_rolled <= 10:
        ...
        cnt = 0
        while i <= times_called:
            cnt = cnt + roll_dice(num_rolled, dice)
            i = i + 1
        if cnt > max_score:
            max_score = cnt
            max_rolled = num_rolled
        num_rolled = num_rolled + 1
    return max_rolled
    # END PROBLEM 9
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    boar_brawl_score = score + boar_brawl(score, opponent_score)
    if sus_points(boar_brawl_score) - score >= threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
